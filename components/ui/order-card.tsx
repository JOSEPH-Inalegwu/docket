...
export default function OrderCard({
  ...
  const getFulfillmentStatusColor = (status: string) => {
    ...
    if (statusLower.includes('unfulfilled')) {
      return 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-400'
    }
  ...
  // Skeleton row – tighter padding on mobile
  if (loading) {
    return (
    ...
    <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
      ...
      <td className="px-4 sm:px-6 py-3 sm:py-4">
        <div className="max-w-[220px] sm:max-w-none">
          ...
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 break-all sm:break-normal">
            {email}
          </p>
        </div>
      </td>
      ...
      <td className="px-4 sm:px-6 py-3 sm:py-4">
        <Badge
          className={`text-xs sm:text-sm px-2 py-1 rounded-full ${getFinancialStatusColor(
            financialStatus,
          )}`}
        >
          {financialStatus}
      ...
      <td className="px-4 sm:px-6 py-3 sm:py-4">
        <Badge
          className={`text-xs sm:text-sm px-2 py-1 rounded-full ${getFulfillmentStatusColor(
